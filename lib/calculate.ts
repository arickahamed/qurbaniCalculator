...
export function calculateDistribution(
  ...
  // Find the group with the highest number of contributors
  const largestGroupIndex = groups.reduce((maxIndex, current, index) => {
  ...
  const results: GroupResult[] = groups.map((group, index) => {
    ...
    // Sorkari (1/3 charity on meat, kolija, tel): single Sam on whole group; multiple Sams on largest group only
